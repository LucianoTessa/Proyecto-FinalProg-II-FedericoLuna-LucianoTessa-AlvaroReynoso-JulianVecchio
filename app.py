...
def iniciar_sesion():
    if request.method == "POST":
        ...
        if user and bcrypt.check_password_hash(user.password, password):
            ...
            return redirect(
                url_for("index", welcome_message=f"Bienvenido, {user.username}!")
            )
        else:
...
def registrar():
    if request.method == "POST":
        ...
        if user:
            flash(
                "El nombre de usuario o el correo electrónico ya están en uso", "danger"
            )
            return redirect(url_for("registrar"))
        ...
        new_user = User(
            username=username, apellido=apellido, email=email, password=hashed_password
        )
        ...
        return redirect(
            url_for(
                "index", welcome_message=f"Usuario registrado exitosamente, {username}!"
            )
        )
